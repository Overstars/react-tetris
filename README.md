...
observer：看了博客，其实关于这一段Immutable还是有疑问的，为什么说JS的这里用到了可持久化数据结构，难道还有保留历史版本的吗？如果没有的话，就是一个简单的渲染树自底向上更新的过程。

update：可持久化的动态示意图有点误解，事实上原来的节点并不应该被删除，因为是要保存历史版本的（因为你不能确保之前的状态被谁引用了），所以找到最小变化节点，作为叶子插入到树上，根据新的class的内容重新建树。而可持久化可以重复利用没有发生变化的节点，在空间上相对于深拷贝优秀了太多（还有一部分拷贝上的时间，O(n)->O(logn)）。
